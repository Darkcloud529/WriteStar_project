...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>  
 ...
 <%@ include file="../includes/header.jsp" %> 
 	...
 	<div id="slider">
 		<div id="slide">
			<c:forEach items="${list}" var="board" begin="0" end="4" varStatus="status">
					<div class="slide_img slide_img${status.count}" data-index='${status.count}'>
						<a class="move" href='/board/get?bno=<c:out value="${board.bno}"/>'>
							<div style='display:hidden' data-path='<c:out value="${board.thumbnail.uploadPath}"/>' data-uuid='<c:out value="${board.thumbnail.uuid}"/>' data-filename='<c:out value="${board.thumbnail.fileName}"/>'></div>
	...
	<div id="content_wrap">
 		<ul id="content">
			<c:forEach items="${list}" var="board" begin="5" end="10">
					<li class="content_img">
						<a class="move" href='/board/get?bno=<c:out value="${board.bno}"/>'>
							<!-- <img src='/display?fileName=<c:out value="${board.thumbnail.uploadPath}"/><c:out value="${board.thumbnail.uuid}"/>_<c:out value="${board.thumbnail.fileName}"/>'><br> -->
							<img src="/display?fileName=<c:out value="${board.thumbnail.uploadPath}"/>\<c:out value="${board.thumbnail.uuid}"/>_<c:out value="${board.thumbnail.fileName}"/>"><br>
                  		</a>
                  		<h1><c:out value="${board.userVO.nickname}"/></h1>
                  		<h2>조회 : <c:out value="${board.hits}"/></h2>
	...
	<script>
		$(document).ready(function(){
			/* var imgWidth = $('#slide>.slide_img').outerWidth();
			...
			function playNext() {
				$('#slide').stop().animate({left: -imgWidth}, 1000, function(){
					$('#slide>.slide_img:first').insertAfter('#slide>.slide_img:last');
					$('#slide').css("left", 0);
					var idx = $('#slide>.slide_img:first').children('img').attr("alt");
					$('.dotList>li').eq(idx-1).addClass('on').siblings('li').removeClass("on");
				});
			}
			function playPrev() {
				$('#slide').css("left", -imgWidth);
				$('#slide>.slide_img:last').insertBefore('#slide>.slide_img:first');
			...
			$('#slide').find('div').each(function(i,e){
				var str = "";
				var path = $(this).find('div').attr("data-path");
				var uuid = $(this).find('div').attr("data-uuid");
				var fileName = $(this).find('div').attr("data-filename");
				
				var fileCallPath =  encodeURIComponent(path+ "/"+uuid +"_"+fileName);
				str += "<img src='/display?fileName="+fileCallPath+"'><br>";
				$(this).children('a:eq(0)').append(str);
			});	
